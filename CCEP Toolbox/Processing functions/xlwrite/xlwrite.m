...
if exist('org.apache.poi.ss.usermodel.WorkbookFactory', 'class') ~= 8 ...
    ...
    javaaddpath(which('dom4j-1.6.1.jar'));
    javaaddpath(which('poi-3.8-20120326.jar'));
    javaaddpath(which('poi-ooxml-3.8-20120326.jar'));
    javaaddpath(which('poi-ooxml-schemas-3.8-20120326.jar'));
    javaaddpath(which('stax-api-1.0.1.jar'));
    javaaddpath(which('xmlbeans-2.3.0.jar'));
    
    %     error('xlWrite:poiLibsNotLoaded',...
    %         'The POI library is not loaded in Matlab.\nCheck that POI jar files are in Matlab Java path!\nCall "AddxlwriteJavaLib.m"');
end
