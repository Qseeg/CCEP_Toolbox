function [DataStruct, Files, StimAnnot, ImportData, Annotations] = CCEPSEEGDataImport(varargin)
...
if nargin == 1 %Check if just the filename is handed to the function
    ...
    for i = 1:2:length(varargin) %Go through every 2nd value of the name-value pair argument
        ...
        elseif ~isempty(regexpi(InputStr,'struct'))  %If the Datastruct is passed, use that instead of re-importing
            DataStruct = varargin{i+1}; %Read in the datastruct
